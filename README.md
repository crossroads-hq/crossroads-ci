...
| `actions/await-codex-review` | Verifies that the exact official Codex account produced a formal review or structured clean-review comment for the pull request's current head. Stale, pending, dismissed, or spoofed evidence does not suppress the fallback. |
...
whose reviewed-commit marker matches the current head. Other comments and
